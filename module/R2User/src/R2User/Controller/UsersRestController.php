...
class UsersRestController extends AbstractRestfulController {
	...
	public function create($data) {
		...
		try {
			if (isset($data['recadastrarSenha'])) {
				$user = $this->em->getRepository($this->repository)->findOneBy(array('activationKey' => $data['activationKey']));
				if ($user) {
					$user->setPassword($data['data']['password']);
					$this->em->merge($user);
					$this->em->flush();
					return new JsonModel(array('success' => true, 'data' => $user->getId()));
				} else {
					return new JsonModel(array('success' => false, 'data' => 'Chave não encontrada'));
				}

				
			}
		} catch(\Exception $e) {
			return new JsonModel(array('success' => false, 'data' => $e->getMessage()));
		}
	...
	private function prepareDataToInsert($data) {
		...
		if (count($person['telephones']) > 0) {
			...
			foreach ($person['telephones'] as $telephone) {
				...
				$telephone['telephoneType'] = $telephone['telephoneType'];
